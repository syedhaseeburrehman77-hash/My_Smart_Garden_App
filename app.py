...
def get_current_location():
    ...
    try:
        # Try secure HTTPS first
        response = requests.get("https://ipapi.co/json/", timeout=5)
        if response.status_code == 200:
            data = response.json()
            return {
                "city": data.get('city', DEFAULT_CITY),
                "country": data.get('country_name', DEFAULT_COUNTRY),
...
if 'user_location' not in st.session_state:
    st.session_state.user_location = {"city": DEFAULT_CITY, "country": DEFAULT_COUNTRY, "lat": None, "lon": None}
    ...
    if user_question:
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤖 AI Botanist is thinking..."):
                ...
                weather_context = f"Current weather in {DEFAULT_CITY}: {current_weather.get('temperature', 25)}°C, {current_weather.get('description', 'clear')}"
